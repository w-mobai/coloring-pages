...
export function ImageGenerator({
  ...
  const [freeTrialsRemaining, setFreeTrialsRemaining] = useState<number>(3);
  ...
  useEffect(() => {
    setIsMounted(true);
    // Load free trials from localStorage
    const storedTrials = localStorage.getItem('freeImageTrials');
    if (storedTrials !== null) {
      setFreeTrialsRemaining(parseInt(storedTrials, 10));
    }
  }, []);
      ...
      try {
        ...
        if (currentStatus === AITaskStatus.SUCCESS) {
          if (imageUrls.length === 0) {
            ...
            // Decrement free trials for non-logged-in users
            if (!user) {
              const newTrials = freeTrialsRemaining - 1;
              setFreeTrialsRemaining(newTrials);
              localStorage.setItem('freeImageTrials', newTrials.toString());
            }
          }
  ...
  const handleGenerate = async () => {
    ...
    if (!user) {
      // Check free trials for non-logged-in users
  ...
  return (
    <section className={cn('pt-8 pb-4', className)}>
          ...
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
            <Card>
              ...
              <CardContent className="space-y-6 pb-8">
                ...
                {/* Credits/Free Trials Info - Below Button */}
                {!isMounted ? (
